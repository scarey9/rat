...
export APP="./.gitlab-scripts/app.yml"
export DEP_TYPE="${1}-deployment"
echo DEP_TYPE
docker build -f kubernetes/templates/Dockerfile-mustache.template kubernetes/templates -t server_generator --build-arg TYPE=$DEP_TYPE
[ $? -eq 0 ]  || exit 1
docker run server_generator >> $APP
[ $? -eq 0 ]  || exit 1
echo "Finished generating template"
echo "Proxy yml is now at: $APP"
...
export SERVICE="./.gitlab-scripts/datacall-service.yml"
export SERV_TYPE="${1}-service"
docker build -f kubernetes/templates/Dockerfile-mustache.template kubernetes/templates -t service_generator --build-arg TYPE=$SERV_TYPE
